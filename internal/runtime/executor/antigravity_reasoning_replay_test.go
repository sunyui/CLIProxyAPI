...
func TestPrepareAntigravityGeminiReasoningReplayPayloadAppendsStaleThoughtSignatureWithoutNullParts(t *testing.T) {
	...
	item := []byte(`{"type":"thought_signature","contentIndex":8,"partIndex":3,"thoughtSignature":"sig-text-signature"}`)
	...
	if got := parts[1].Get("thoughtSignature").String(); got != "sig-text-signature" {
		t.Fatalf("thoughtSignature = %q, want sig-text-signature; body=%s", got, out)
	}
}
