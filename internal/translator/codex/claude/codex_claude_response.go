...
const codexReasoningDisplaySentinel = "<!-- -->"
...
func ConvertCodexResponseToClaude(_ context.Context, _ string, originalRequestRawJSON, _ []byte, rawJSON []byte, param *any) [][]byte {
	...
	case "response.reasoning_summary_text.delta":
		thinking := sanitizeCodexReasoningDisplayDelta(params, rootResult.Get("delta").String())
		if thinking != "" {
			output = appendCodexThinkingDelta(output, params.BlockIndex, thinking)
		}
...
func sanitizeCodexReasoningDisplayDelta(params *ConvertCodexResponseToClaudeParams, delta string) string {
	combined := params.ThinkingDisplayPending + delta
	combined = strings.ReplaceAll(combined, codexReasoningDisplaySentinel, "")

	pendingLength := 0
	maxPending := min(len(combined), len(codexReasoningDisplaySentinel)-1)
	for length := maxPending; length > 0; length-- {
		if strings.HasSuffix(combined, codexReasoningDisplaySentinel[:length]) {
			pendingLength = length
			break
		}
	}

	params.ThinkingDisplayPending = strings.Clone(combined[len(combined)-pendingLength:])
	return combined[:len(combined)-pendingLength]
}

func appendCodexThinkingDelta(output []byte, blockIndex int, thinking string) []byte {
	template := []byte(`{"type":"content_block_delta","index":0,"delta":{"type":"thinking_delta","thinking":""}}`)
	template, _ = sjson.SetBytes(template, "index", blockIndex)
	template, _ = sjson.SetBytes(template, "delta.thinking", thinking)
	return translatorcommon.AppendSSEEventBytes(output, "content_block_delta", template, 2)
}
...
func finalizeCodexThinkingBlock(params *ConvertCodexResponseToClaudeParams) []byte {
	...
	output := make([]byte, 0, 256)
	if params.ThinkingDisplayPending != "" {
		output = appendCodexThinkingDelta(output, params.BlockIndex, params.ThinkingDisplayPending)
		params.ThinkingDisplayPending = ""
	}
	if params.ThinkingSignature != "" {
