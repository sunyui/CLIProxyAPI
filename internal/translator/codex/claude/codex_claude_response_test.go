...
func TestConvertCodexResponseToClaude_StreamReasoningSummarySentinel(t *testing.T) {
	const marker = "<!-- -->"

	t.Run("complete and repeated markers", func(t *testing.T) {
		got, outputs := convertCodexReasoningDeltas(t, []string{"alpha" + marker + "omega" + marker})
		if got != "alphaomega" {
			t.Fatalf("thinking = %q, want alphaomega", got)
		}
		assertNoEmptyThinkingDelta(t, outputs)
	})

	t.Run("every split boundary", func(t *testing.T) {
		for split := 1; split < len(marker); split++ {
			t.Run(string(rune('0'+split)), func(t *testing.T) {
				got, outputs := convertCodexReasoningDeltas(t, []string{"before" + marker[:split], marker[split:] + "after"})
				if got != "beforeafter" {
					t.Fatalf("split %d thinking = %q, want beforeafter", split, got)
				}
				assertNoEmptyThinkingDelta(t, outputs)
			})
		}
	...
	t.Run("marker only does not emit empty delta", func(t *testing.T) {
		got, outputs := convertCodexReasoningDeltas(t, []string{marker})
		if got != "" {
	...
	t.Run("similar comments and incomplete prefix are preserved", func(t *testing.T) {
		got, _ := convertCodexReasoningDeltas(t, []string{"<!--x-->", "<!--  -->", "<!--"})
		if got != "<!--x--><!--  --><!--" {
			t.Fatalf("thinking = %q", got)
		}
	})

	t.Run("pending prefix flushes before signature", func(t *testing.T) {
		got, outputs := convertCodexReasoningDeltas(t, []string{"text<!--"})
		if got != "text<!--" {
			t.Fatalf("thinking = %q, want text<!--", got)
		}
		if gotOrder := codexClaudeEventOrder(outputs); gotOrder != "thinking_start,thinking_delta,thinking_delta,signature_delta,thinking_stop" {
			t.Fatalf("event order = %s", gotOrder)
		}
	})

	t.Run("tool index follows marker only thinking", func(t *testing.T) {
		ctx := context.Background()
		var param any
		chunks := [][]byte{
			[]byte(`data: {"type":"response.output_item.added","item":{"type":"reasoning","encrypted_content":"sig"}}`),
			[]byte(`data: {"type":"response.reasoning_summary_part.added"}`),
			[]byte(`data: {"type":"response.reasoning_summary_text.delta","delta":"<!-- -->"}`),
			[]byte(`data: {"type":"response.output_item.done","item":{"type":"reasoning"}}`),
			[]byte(`data: {"type":"response.output_item.added","output_index":1,"item":{"type":"function_call","call_id":"call_1","name":"Read"}}`),
		}
		outputs := convertCodexClaudeChunks(ctx, chunks, &param)
		for _, out := range outputs {
			for _, line := range strings.Split(string(out), "\n") {
				if !strings.HasPrefix(line, "data: ") {
					continue
				...
				data := gjson.Parse(strings.TrimPrefix(line, "data: "))
				if data.Get("type").String() == "content_block_start" && data.Get("content_block.type").String() == "tool_use" && data.Get("index").Int() != 1 {
					t.Fatalf("tool index = %d, want 1", data.Get("index").Int())
				}
			}
		}
	})

	t.Run("final text is unchanged", func(t *testing.T) {
		ctx := context.Background()
		var param any
		outputs := convertCodexClaudeChunks(ctx, [][]byte{
			[]byte(`data: {"type":"response.content_part.added","part":{"type":"output_text"}}`),
			[]byte(`data: {"type":"response.output_text.delta","delta":"final<!-- -->text"}`),
		}, &param)
		if got := collectCodexClaudeDelta(outputs, "text_delta", "text"); got != "final<!-- -->text" {
			t.Fatalf("text = %q", got)
		}
	})
}

func convertCodexReasoningDeltas(t *testing.T, deltas []string) (string, [][]byte) {
	t.Helper()
	ctx := context.Background()
	var param any
	chunks := [][]byte{
	...
	for _, delta := range deltas {
		chunks = append(chunks, []byte(`data: {"type":"response.reasoning_summary_text.delta","delta":`+string(mustJSONMarshal(t, delta))+`}`))
	}
	chunks = append(chunks, []byte(`data: {"type":"response.output_item.done","item":{"type":"reasoning"}}`))
	outputs := convertCodexClaudeChunks(ctx, chunks, &param)
	return collectCodexClaudeDelta(outputs, "thinking_delta", "thinking"), outputs
}

func convertCodexClaudeChunks(ctx context.Context, chunks [][]byte, param *any) [][]byte {
	var outputs [][]byte
	for _, chunk := range chunks {
...
func collectCodexClaudeDelta(outputs [][]byte, deltaType, field string) string {
	...
	for _, out := range outputs {
		for _, line := range strings.Split(string(out), "\n") {
			...
			data := gjson.Parse(strings.TrimPrefix(line, "data: "))
			if data.Get("delta.type").String() == deltaType {
				text.WriteString(data.Get("delta." + field).String())
			}
		}
	}
	return text.String()
...
func assertNoEmptyThinkingDelta(t *testing.T, outputs [][]byte) {
	t.Helper()
	for _, out := range outputs {
		for _, line := range strings.Split(string(out), "\n") {
			if !strings.HasPrefix(line, "data: ") {
			...
			data := gjson.Parse(strings.TrimPrefix(line, "data: "))
			if data.Get("delta.type").String() == "thinking_delta" && data.Get("delta.thinking").String() == "" {
				t.Fatalf("empty thinking delta: %s", line)
			}
		}
	}
}

func codexClaudeEventOrder(outputs [][]byte) string {
	var events []string
	for _, out := range outputs {
		for _, line := range strings.Split(string(out), "\n") {
			if !strings.HasPrefix(line, "data: ") {
			...
			case "content_block_delta":
				events = append(events, data.Get("delta.type").String())
			case "content_block_stop":
